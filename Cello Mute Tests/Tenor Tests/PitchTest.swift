...
class PitchTest: TestCase {
    ...
    func testClosestNote() {
        assertEqual(Pitch(frequency: 220)?.closestNote(using: .sharp), Note(.a(.natural), 3))
        assertEqual(Pitch(frequency: 440)?.closestNote(using: .flat), Note(.a(.natural), 4))
        assertEqual(Pitch(frequency: 880)?.closestNote(using: .flat), Note(.a(.natural), 5))
    }

    func assertEqual(_ lhs: Note?, _ rhs: Note?) {
        guard let lhs = lhs, let rhs = rhs else {
            return
        }

        XCTAssertEqual(lhs.octave, rhs.octave)
        XCTAssertEqual(lhs.name.semitone, rhs.name.semitone)
    }
